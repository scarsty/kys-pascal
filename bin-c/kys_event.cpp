...
void talk_1(const std::string& talkstr, int headnum, int dismode)
{
    ...
    for (size_t i = 0; i < lines.size(); i++)
    {
        ...
        if (li >= 4 && i < lines.size() - 1)
        {
            ...
            do {
                key = WaitAnyKey();
            } while (key == SDLK_LEFT || key == SDLK_RIGHT || key == SDLK_UP || key == SDLK_DOWN);
            Redraw();
            DrawRectangleWithoutFrame(screen, 0, diagy - 10, CENTER_X * 2, 120, 0, 60);
            if (headx > 0)
            {
...
void instruct_3(int list[])
{
    ...
    if (list[0] == CurScene && prePic != DData[list[0]][list[1]][5])
    {
        NeedRefreshScene = 1;
    }
}
...
void instruct_26(int snum, int en, int add1, int add2, int add3)
{
    ...
    DData[snum][en][2] += add1;
    DData[snum][en][3] += add2;
    DData[snum][en][4] += add3;
    if (snum == CurScene)
    {
        InitialScene(0);
        NeedRefreshScene = 0;
...
void instruct_27(int en, int beginpic, int endpic)
{
    ...
    else
    {
        ...
        {
            CheckBasicEvent();
            DData[CurScene][en][5] = i;
            DData[CurScene][en][6] = i;
            DData[CurScene][en][7] = i;
            InitialScene(1);
            SDL_Delay(20);
            DrawScene();
            UpdateScreen(screen, 0, 0, screen->w, screen->h);
...
int instruct_29(int rnum, int r1, int r2, int jump1, int jump2)
{
    if (MODVersion == 41)
    {
        return (Rrole[rnum].Attack >= r1) ? jump1 : jump2;
    }
...
int instruct_42(int jump1, int jump2)
{
    for (int i = 0; i < 6; i++)
    {
        if (TeamList[i] >= 0 && Rrole[TeamList[i]].Sexual == 1)
        {
            return jump1;
        }
    }
...
int instruct_60(int snum, int en, int pic, int jump1, int jump2)
{
    ...
    if (DData[snum][en][5] == pic || DData[snum][en][6] == pic || DData[snum][en][7] == pic)
    {
        return jump1;
    }
    return jump2;
...
void instruct_64()
{
    ...
    if (amount >= 1)
    {
        ...
        {
            menu = list[menu];
            int price = RShop[shopnum].Price[menu];
...
int instruct_50e(int code, int e1, int e2, int e3, int e4, int e5, int e6)
{
    ...
    case 1:
        t1 = CutRegion(e3 + e_GetValue(0, e1, e4));
    ...
    break;
    case 36:
    {    // Draw a string with background then pause
        e3 = e_GetValue(0, e1, e3);
        e4 = e_GetValue(1, e1, e4);
        e5 = e_GetValue(2, e1, e5);
        p = (char*)&x50[e2];
    ...
    case 50:
    {    // Enter name for items, magics and roles
        e2 = e_GetValue(0, e1, e2);
        e3 = e_GetValue(1, e1, e3);
        e4 = e_GetValue(2, e1, e4);
        e5 = e_GetValue(3, e1, e5);
    ...
    break;
    case 52:
    {    // Judge someone grasp some magic
        e2 = e_GetValue(0, e1, e2);
        e3 = e_GetValue(1, e1, e3);
        e4 = e_GetValue(2, e1, e4);
        x50[0x7000] = 1;
...
void NewTalk(int headnum, int talknum, int namenum, int place, int showhead, int color, int frame, const std::string& content, const std::string& disname)
{
    ...
    if (SkipTalk == 0)
    {
        ...
        if (false)
        {
            WaitAnyKey();    // skipSync is local, already handled
        }
    }
}
