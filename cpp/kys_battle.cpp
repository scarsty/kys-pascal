...
void Attack(int bnum) {
    ...
    while (true) {
        ...
        int magIdx = 0;
        for (int i = 0; i < 10; i++) {
            if (Rrole[rnum].Magic[i] == mnum) { magIdx = i; break; }
        }
        ...
        if (Brole[bnum].Acted == 1) {
            ...
            for (int i1 = 0; i1 <= (Rrole[rnum].AttTwice > 0 ? 1 : 0); i1++) {
                ...
                if (Rrole[rnum].MagLevel[magIdx] > 999) Rrole[rnum].MagLevel[magIdx] = 999;
...
int SelectMagic(int rnum) {
    ...
    while (SDL_WaitEvent(&e)) {
        ...
        if (e.type == SDL_EVENT_KEY_UP) {
            if (e.key.key == SDLK_RETURN || e.key.key == SDLK_SPACE) break;
            if (e.key.key == SDLK_ESCAPE) { result = -1; break; }
            if (e.key.key == SDLK_UP) {
                menu--;
                if (menu < 0) menu = maxMenu;
                ShowMagicMenu();
            }
            ...
            if (e.button.button == SDL_BUTTON_RIGHT) { result = -1; break; }
...
void SetAminationPosition2(int bx1, int by1, int ax1, int ay1, int mode, int step, int range) {
    ...
    switch (mode) {
        ...
        case 1: { // 线
            int i = 1;
            int i1 = (ax1 > bx1) ? 1 : (ax1 < bx1) ? -1 : 0;
            int i2 = (ay1 > by1) ? 1 : (ay1 < by1) ? -1 : 0;
            if (i1 > 0) step = std::min(63 - bx1, step);
            if (i2 > 0) step = std::min(63 - by1, step);
            if (i1 < 0) step = std::min(bx1, step);
            if (i2 < 0) step = std::min(by1, step);
            if (i1 == 0 && i2 == 0) step = 0;
            while (i <= step) {
...
void PlayMagicAmination(int bnum, int enumv, int ForTeam, int mode) {
    ...
    for (int i1 = 0; i1 <= 63; i1++)
        for (int i2 = 0; i2 <= 63; i2++)
            if (BField[4][i1][i2] > 0) {
                if (BField[4][i1][i2] > max_) max_ = BField[4][i1][i2];
                if (BField[4][i1][i2] < min_) min_ = BField[4][i1][i2];
            }
        ...
        if (BField[4][Brole[i].X][Brole[i].Y] != 0 && Brole[bnum].Team != Brole[i].Team && Brole[i].Dead == 0) {
            ...
            if (Rmagic[mnum].HurtType == 0) {
                ...
                if (Rrole[Brole[i].rnum].CurrentHP <= 0)
                    Brole[bnum].ExpGot += hurt / 2;
                if (Brole[bnum].ExpGot < 0) Brole[bnum].ExpGot = 32767;
            }
            // 内力伤害
...
int CalHurtValue(int bnum1, int bnum2, int mnum, int level) {
    ...
    int mhurt = 0;
    if (level > 0) mhurt = Rmagic[mnum].Attack[level - 1];
    ...
    switch (Rmagic[mnum].MagicType) {
        case 1: att += Rrole[rnum1].Fist;   def += Rrole[rnum2].Fist;   break;
        case 2: att += Rrole[rnum1].Sword;  def += Rrole[rnum2].Sword;  break;
        case 3: att += Rrole[rnum1].Knife;  def += Rrole[rnum2].Knife;  break;
        case 4: att += Rrole[rnum1].Unusual; def += Rrole[rnum2].Unusual; break;
    }
    ...
    if (result <= 0 || level <= 0) result = rand() % 10 + 1;
...
void PlayActionAmination(int bnum, int mode, int mnum) {
    ...
    if (Rrole[rnum].AmiFrameNum[mode] > 0) {
        ...
        if (beginPic < 0 || beginPic > endPic) { beginPic = 0; endPic = 0; }
...
bool TeamModeMenu(int bnum) {
    ...
    while (SDL_WaitEvent(&e)) {
        ...
        if (e.type == SDL_EVENT_KEY_UP) {
            if (e.key.key == SDLK_RETURN || e.key.key == SDLK_SPACE) break;
            if (e.key.key == SDLK_ESCAPE) { result = false; break; }
            if (e.key.key == SDLK_UP) {
                menu--;
                if (menu == -1) menu = -2;
            ...
            if (e.button.button == SDL_BUTTON_RIGHT) { result = false; break; }
        } else if (e.type == SDL_EVENT_MOUSE_MOTION) {
            int xm, ym;
            if (MouseInRegion(x, y, w, amount * 22 + 28, xm, ym)) {
                int menup = menu;
                ...
                if (menu < 0) menu = 0;
                if (menu >= amount) menu = -2;
                if (menup != menu) ShowTeamModeMenu();
            }
        }
    }
